...
const finalDir = path.join(project, "out", "alton-towers-accurate-paths-v071-2026-08-02");
const worldSource = path.join(finalDir, "alton-towers_1to1.mcworld");
const worldOutput = path.join(project, "out", "Alton_Towers_1to1_Accurate_Paths_v0.7.1_2026-08-02.mcworld");
const sourceOutput = path.join(project, "out", "ThemePark_Map_v0.7.1_Accurate_Paths.zip");
const evidenceOutput = path.join(project, "out", "Alton_Towers_1to1_Accurate_Paths_Evidence_v0.7.1_2026-08-02.zip");

const validation = JSON.parse(await readFile(path.join(finalDir, "WORLD_VALIDATION.json"), "utf8"));
const fidelity = JSON.parse(await readFile(path.join(finalDir, "fidelity.json"), "utf8"));
if (validation.status !== "passed") throw new Error("world validation did not pass");
if (fidelity.surfaces?.widthMode !== "inferred") throw new Error("release did not use inferred path widths");
if (fidelity.surfaces?.compiledWidthCoverage !== 1) throw new Error("release has uncompiled linear-route widths");

await copyFile(worldSource, worldOutput);

const sourceRoot = "ThemePark_Map_v0.7.1_Accurate_Paths";
const sourceArchive = {};
...
const evidenceRoot = "Alton_Towers_1to1_Accurate_Paths_Evidence_v0.7.1_2026-08-02";
...
for (const required of ["/src/lib/fidelity.mjs", "/test/core.test.mjs", "/UNIVERSAL_SOURCE_ARCHITECTURE.md"]) {
