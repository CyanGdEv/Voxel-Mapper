...
const finalDir = path.join(project, "out", "alton-towers-universal-v07-2026-08-02");
const worldSource = path.join(finalDir, "alton-towers_1to1.mcworld");
const worldOutput = path.join(project, "out", "Alton_Towers_1to1_Universal_Fidelity_v0.7.0_2026-08-02.mcworld");
const sourceOutput = path.join(project, "out", "ThemePark_Map_v0.7.0_Universal_Fidelity.zip");
const evidenceOutput = path.join(project, "out", "Alton_Towers_1to1_Universal_Fidelity_v0.7.0_2026-08-02.zip");

await copyFile(worldSource, worldOutput);

const sourceRoot = "ThemePark_Map_v0.7.0_Universal_Fidelity";
const sourceArchive = {};
...
const evidenceRoot = "Alton_Towers_1to1_Universal_Fidelity_v0.7.0_2026-08-02";
...
for (const required of ["/src/lib/fidelity.mjs", "/UNIVERSAL_SOURCE_ARCHITECTURE.md"]) {
  if (!sourceEntries.some((entry) => entry.endsWith(required))) throw new Error(`source archive missing ${required}`);
}
for (const required of ["/fidelity.json", "/WORLD_VALIDATION.json", "/portable-ride-profiles.geojson"]) {
