...
export async function validateParkGeneration(options) {
  ...
  pass(chunkRatio >= 0.9, `Chunk roster is too small for the requested bbox: ${actualChunks} vs ~${expectedChunks} expected (${chunkRatio.toFixed(3)}x)`);
  pass(chunkRatio <= 1.35, `Chunk roster is unexpectedly large for the requested bbox: ${actualChunks} vs ~${expectedChunks} expected (${chunkRatio.toFixed(3)}x)`);
  pass(Number(plan.chunkCount || 0) === actualChunks, "World shard plan chunk count does not match generated world chunk count");
  pass(Number(manifest.chunks || 0) === actualChunks, "World manifest chunk count does not match generated world chunk count");
  pass(generation.worldValidation === "passed", `World validation is ${generation.worldValidation || "missing"}, not passed`);
  pass(manifest.validation?.status === "passed", "Assembled world manifest validation did not pass");
  pass(Number(manifest.validation?.chunksVerified || 0) === actualChunks, "Merged LevelDB did not verify every planned chunk");
  ...
  if (previewPath) {
    ...
    pass(previewCoverage >= 0.95, `Top-down preview renders only ${(previewCoverage * 100).toFixed(1)}% of final map features`);
  }

  const qa = {
    pathGeometry: files.some((filename) => path.basename(filename) === "path-geometry-qa.geojson"),
    pathTopology: files.some((filename) => path.basename(filename) === "path-topology-qa.geojson"),
    orthophoto: files.some((filename) => path.basename(filename) === "orthophoto-qa.geojson")
  };
  pass(qa.pathGeometry, "Path geometry QA GeoJSON is missing");
  pass(qa.pathTopology, "Path topology QA GeoJSON is missing");
  warn(qa.orthophoto, "Orthophoto QA GeoJSON is missing; imagery assistance may have been unavailable");

  const elevationProvider = evidence.source?.elevation?.provider || evidence.source?.elevation?.sourceKind || null;
  pass(Boolean(elevationProvider) && elevationProvider !== "none", "No elevation/LiDAR provider contributed to the generated park");
  pass(Boolean(evidence.source?.osm), "OSM acquisition evidence is missing");
  warn(Boolean(evidence.source?.planning), "Planning acquisition evidence is missing");
  ...
  const report = {
    ...
    sources: {
      elevationProvider,
      planningApplications: Number(build.stats?.planningApplications || 0),
      planningAuthorityAppliedAttributes: Number(generation.planningAuthorityAppliedAttributes || build.stats?.planningAuthorityAppliedAttributes || 0),
...
async function selectFinalMap(files) {
  const candidates = files.filter((filename) => filename.endsWith(".geojson") && !path.basename(filename).includes("-qa"));
  let selected = null;
  let score = -1;
  for (const filename of candidates) {
    try {
      ...
      // Ignore unrelated malformed/partial QA files here; explicit required
      // artifacts are checked separately.
    }
  }
  return selected;
}
...
function geometryLengthM(geometry) {
  ...
  if (geometry.type === "MultiLineString") return geometry.coordinates.reduce((sum, line) => sum + lineLengthM(line), 0);
  if (geometry.type === "Polygon") return geometry.coordinates.reduce((sum, ring) => sum + lineLengthM(ring), 0);
  if (geometry.type === "MultiPolygon") return geometry.coordinates.flat().reduce((sum, ring) => sum + lineLengthM(ring), 0);
  return 0;
}

function lineLengthM(line) {
  let total = 0;
  for (let index = 1; index < (line || []).length; index += 1) total += haversineM(line[index - 1], line[index]);
  return total;
}
...
function isRenderableGeometry(geometry) {
  return Boolean(geometry && ["Point", "LineString", "Polygon", "MultiLineString", "MultiPolygon"].includes(geometry.type));
}
