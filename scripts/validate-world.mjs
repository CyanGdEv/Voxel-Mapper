...
assert.equal(labelIndex.mode, "markers");
...
for (const forbidden of ["minecraft:deepslate_tiles", "minecraft:cracked_deepslate_tiles"]) {
...
for (const required of ["minecraft:standing_sign", "minecraft:yellow_concrete"]) {
  assert.ok(paletteNames.has(required), `marker world is missing ${required}`);
}
...
for (const title of ["THEMEPARK MAP", "TRACK COLOURS", "MORE TRACK COLOURS", "PATH SURFACES", "TERRAIN DETAIL", "RIDE STRUCTURES", "NOT LIVE PARK INFO"]) {
...
const rideInformationTexts = informationTexts.filter((text) => ![
  "THEMEPARK MAP", "TRACK COLOURS", "MORE TRACK COLOURS", "PATH SURFACES", "TERRAIN DETAIL", "RIDE STRUCTURES", "NOT LIVE PARK INFO"
...
const report = {
  ...
  playerInformation: {
    ...
    requiredBoardTitles: ["THEMEPARK MAP", "TRACK COLOURS", "MORE TRACK COLOURS", "PATH SURFACES", "TERRAIN DETAIL", "RIDE STRUCTURES", "NOT LIVE PARK INFO"],
    status: "passed"
  },
  rideStructures: {
...
function markdownReport(report) {
  return `# Alton Towers marker-world validation
...
- Legacy shell/roof palette materials found: none
