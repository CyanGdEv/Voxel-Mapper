...
export async function fuseAdditionalMapSources(features, projector, options = {}) {
  ...
  const summary = {
    schemaVersion: 1,
    status: options.overture?.length || options.publicData?.length ? "active" : "osm-only",
    policy: {
  ...
  for (const filename of options.publicData || []) {
    ...
    for (const [index, raw] of collection.features.entries()) {
      summary.publicData.considered += 1;
      const feature = publicDataToFeature(raw, projector, resolved, index, defaults);
      const replaces = raw.properties?.replaces;
      if (replaces) {
        const replaced = removeById(features, replaces);
        if (!replaced) {
          summary.publicData.missingReplacementTargets += 1;
        } else {
          summary.publicData.replaced += 1;
          feature.source.replaces = replaces;
        }
      }
      features.push(feature);
      summary.publicData.accepted += 1;
      increment(summary.publicData.kinds, feature.kind);
      increment(summary.providers, feature.source.provider);
    }
  }

  summary.acceptedFeatures = summary.overture.accepted + summary.publicData.accepted;
  summary.files = summary.overture.files.length + summary.publicData.files.length;
