...
export async function buildBedrockAddon({ parkName, slug, compilation, outputDir, options = {} }) {
  ...
  await writeJson(path.join(packRoot, "manifest.json"), {
    ...
    metadata: {
      authors: ["ThemePark Map evidence compiler"],
      generated_with: { themepark_map: [0, 6, 0] },
      license: "MIT"
    }
  });
...
function tell(target, message) {
  try { target?.sendMessage?.(\`§b[ThemePark Map]§r \${message}\`); }
  catch { console.warn(\`[ThemePark Map] \${message}\`); }
}
  ...
  try {
    ...
    for (const part of PARK.parts) {
      for (const chunk of part) {
        ...
        catch (error) { state.errors += 1; console.warn(\`[ThemePark Map] ticking area: \${error}\`); }
    ...
    for (const sign of PARK.signs || []) {
      ...
      try {
        ...
        state.errors += 1;
        console.warn(\`[ThemePark Map] sign failed: \${error}\`);
      }
      signsDone += 1;
