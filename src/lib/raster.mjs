...
function generateVegetationCoverCandidates(context) {
  ...
  if (!density || remainingModels <= 0) return [];
  ...
  const target = Math.min(
    remainingModels,
    Math.max(1, Math.round((eligibleCells * density) / 100))
  );
